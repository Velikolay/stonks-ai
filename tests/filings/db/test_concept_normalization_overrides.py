...
class TestConceptNormalizationOverridesOperations:
    ...
    async def test_create_override(self, db, sample_override):
        """Test creating a new concept normalization override."""
        created = await db.concept_normalization_overrides.create(sample_override)
    ...
    async def test_create_duplicate_override(self, db, sample_override):
        """Test creating duplicate override raises error."""
        await db.concept_normalization_overrides.create(sample_override)

        with pytest.raises(ValueError, match="already exists"):
            await db.concept_normalization_overrides.create(sample_override)

    async def test_get_by_key(self, db, sample_override):
        """Test retrieving override by concept and statement."""
        created = await db.concept_normalization_overrides.create(sample_override)
        retrieved = await db.concept_normalization_overrides.get_by_key(
    ...
    async def test_get_by_key_not_found(self, db):
    ...
    async def test_list_all(self, db, sample_override):
        ...
        await db.concept_normalization_overrides.create(override1)
        await db.concept_normalization_overrides.create(override2)
        await db.concept_normalization_overrides.create(override3)

        all_overrides = await db.concept_normalization_overrides.list_all(company_id=0)
    ...
    async def test_list_all_with_statement_filter(self, db):
        ...
        income_statement_overrides = await db.concept_normalization_overrides.list_all(
        ...
        balance_sheet_overrides = await db.concept_normalization_overrides.list_all(
    ...
    async def test_update_override(self, db, sample_override):
        ...
        updated = await db.concept_normalization_overrides.update(
    ...
    async def test_update_override_partial(self, db, sample_override):
    ...
    async def test_update_override_not_found(self, db):
    ...
    async def test_update_override_no_changes(self, db, sample_override):
    ...
    async def test_delete_override(self, db, sample_override):
        """Test deleting an override."""
        created = await db.concept_normalization_overrides.create(sample_override)

        deleted = await db.concept_normalization_overrides.delete(
            concept=created.concept, statement=created.statement, company_id=0
        )
    ...
    async def test_delete_override_not_found(self, db):
    ...
    async def test_create_with_abstract_concept(
        ...
        await db.concept_normalization_overrides.create(parent)

        # Then create the child
        child = await db.concept_normalization_overrides.create(
            sample_override_with_parent
        )

        assert child is not None
        assert child.abstract_concept == "us-gaap:ParentConcept"

    async def test_create_with_invalid_abstract_concept(self, db, sample_override):
        ...
        with pytest.raises(ValueError, match="invalid abstract_concept"):
            await db.concept_normalization_overrides.create(
                override_with_invalid_parent
            )

    async def test_is_abstract_values(self, db):
        ...
        created_false = await db.concept_normalization_overrides.create(override_false)
        created_true = await db.concept_normalization_overrides.create(override_true)
